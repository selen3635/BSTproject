 ...
 *
 *
 *  You MUST output size, then height, then prompt the user to search, 
 *  as above.
 *
 *
 *
...
int main(int argc, char* argv[])
{
  ...
  while ( tryAgain == "y" ) {
    ...
    auto result = myTree.find(name);
    BST<std::string>::iterator en = myTree.end();
    if( result != en ) {
    /* To tell the user that actor/actress name was found: */
    cout << name << " found!" << "\n";
